...
export class IndicatorManager {
  ...
  async addIndicatorSeries(id) {
    ...
    // Do not render hidden indicators
    ...
    // Robustly capture the pane HTML element so the indicator panel appears immediately
    await this.ensurePaneHtmlElement(id, paneIndex);
  }

  async ensurePaneHtmlElement(id, paneIndex, attempts = 12, delayMs = 100) {
    for (let i = 0; i < attempts; i++) {
      try {
        const paneEl = await this.chartManager.getPaneHtmlElement(paneIndex);
        if (paneEl) {
          this.indicatorsStore.updateIndicatorPaneElement(id, paneEl);
          return true;
        }
      } catch {}
      if (i < attempts - 1) {
    ...
    // Fallback: try the store-wide updater
    await this.updateMissingPaneHtmlElements();
    return false;
  }
